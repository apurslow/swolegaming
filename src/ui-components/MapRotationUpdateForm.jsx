...
export default function MapRotationUpdateForm(props) {
  const {
    id: idProp,
    mapRotation: mapRotationModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = mapRotationRecord
      ? { ...initialValues, ...mapRotationRecord }
      : initialValues;
    setMapName(cleanValues.MapName);
    setMapSource(cleanValues.MapSource);
    setMapAuthor(cleanValues.MapAuthor);
    setImgSRC(cleanValues.ImgSRC);
    setLastUpdated(cleanValues.LastUpdated);
    setUGC(cleanValues.UGC);
    setErrors({});
  };
  const [mapRotationRecord, setMapRotationRecord] =
    React.useState(mapRotationModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(MapRotation, idProp)
        : mapRotationModelProp;
      setMapRotationRecord(record);
    };
    queryData();
  }, [idProp, mapRotationModelProp]);
  React.useEffect(resetStateValues, [mapRotationRecord]);
  const validations = {
    MapName: [],
    MapSource: [{ type: "URL" }],
    MapAuthor: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            MapRotation.copyOf(mapRotationRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "MapRotationUpdateForm")}
      {...rest}
    >
      <TextField
        label="Map name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || mapRotationModelProp)}
